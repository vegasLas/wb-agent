...
const consoleFormat = printf(({ level, message, timestamp, label, service, ...metadata }) => {
  const color = levelColors[level as keyof typeof levelColors] || '';
  const context = cleanLabel(label as string) || cleanLabel(service as string) || 'app';
...
export const requestLoggerFormat = combine(
  timestamp({ format: 'DD-MM HH:mm' }),
  printf(({ level, message, timestamp, label, service, method, url, statusCode, duration }) => {
    const context = cleanLabel(label as string) || cleanLabel(service as string) || 'app';

  ...
  >
    <div class="max-h-[70vh] overflow-auto">
      ...
      <div v-else-if="excelItems.length > 0">
        ...
        <div class="mb-4 p-4 rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800">
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap gap-6 text-sm">
              <div>
                <span class="text-gray-500 dark:text-gray-400">Всего товаров:</span>
                <span class="ml-1 font-medium text-gray-900 dark:text-gray-100">{{ excelItems.length }}</span>
              </div>
              <div v-if="participatingCount > 0">
                <span class="text-gray-500 dark:text-gray-400">Участвует:</span>
                <span class="ml-1 font-medium text-green-600 dark:text-green-400">{{ participatingCount }}</span>
              </div>
              <div v-if="notParticipatingCount > 0">
                <span class="text-gray-500 dark:text-gray-400">Не участвует:</span>
                <span class="ml-1 font-medium text-gray-600 dark:text-gray-400">{{ notParticipatingCount }}</span>
              </div>
            </div>
        ...
        >
          ...
          >
            <template #body="slotProps">
              <span 
                class="text-sm"
                :class="slotProps.data['Товар уже участвует в акции'] === 'Да' ? 'text-green-600 dark:text-green-400 font-medium' : 'text-gray-500 dark:text-gray-400'"
              >
          ...
          >
            <template #body="slotProps">
              <span
                class="text-sm"
                :class="getStockClass(slotProps.data['Остаток товара на складах Wb (шт.)'])"
              >
          ...
          >
            <template #body="slotProps">
              <span
                class="text-sm"
                :class="getStockClass(slotProps.data['Остаток товара на складе продавца Wb (шт.)'])"
              >
...
const availableColumns: ColumnConfig[] = [
  ...
  { field: 'Текущая розничная цена', header: 'Тек. цена', defaultVisible: true },
  { field: 'Плановая цена для акции', header: 'Цена в акции', defaultVisible: true },
  { field: 'Текущая скидка на сайте, %', header: 'Тек. скидка', defaultVisible: true },
  { field: 'Загружаемая скидка для участия в акции', header: 'Загр. скидка', defaultVisible: true },
  { field: 'Товар уже участвует в акции', header: 'В акции', defaultVisible: true },
  { field: 'Остаток товара на складах Wb (шт.)', header: 'Остаток WB', defaultVisible: true },
  { field: 'Остаток товара на складе продавца Wb (шт.)', header: 'Остаток продавца', defaultVisible: false },
  { field: 'Оборачиваемость', header: 'Оборачиваемость', defaultVisible: false },
  { field: 'Количество дней на сайте', header: 'Дней на сайте', defaultVisible: false },
...
const selectedColumns = useLocalStorage<string[]>('promotions-selected-columns', [...defaultColumns]);
...
const dialogHeader = computed(() => {
  return props.promotionName ? `Участники: ${props.promotionName}` : 'Участники акции';
});

// Count participating items (Товар уже участвует в акции === 'Да')
const participatingCount = computed(() => {
  return props.excelItems.filter(item => item['Товар уже участвует в акции'] === 'Да').length;
});
...
function getStatusSeverity(status: string): string {
  ...
  if (lowerStatus.includes('не участвует') || lowerStatus.includes('отклонено')) {
    return 'secondary';
  }
  return 'info';
}

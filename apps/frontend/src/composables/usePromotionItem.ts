...
import type { PromotionItem, PromotionDetail, PromotionParticipation } from '../types';

export type ParticipationStatus = 'PARTICIPATING' | 'WILL_PARTICIPATE' | 'AVAILABLE' | 'SKIPPED' | 'UNKNOWN';
export type PromotionType = 'AUTO_ACTION' | 'REGULAR';
export type Severity = 'success' | 'info' | 'warn' | 'secondary' | 'contrast' | 'danger' | undefined;
...
export interface UsePromotionDetailReturn {
  ...
  rangingLevels: ComputedRef<Array<{ nomenclatures: number; coefficient: number }>>;
...
export function usePromotionItem(
  ...
  const type = computed((): PromotionType => 
...
export function usePromotionDetail(
  ...
  const formattedDescription = computed(() => item()?.formattedDescription ?? '');
  ...
  // Stats
  const participationPercentage = computed(() => item()?.participationPercentage ?? 0);
